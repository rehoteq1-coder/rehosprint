...
(() => {
  ...
  function getFilteredSchoolEntries() {
    let entries = Object.entries(schoolsData);
    if (currentFilter !== "all") {
      entries = entries.filter(([, s]) => s.status === currentFilter);
    }
  ...
  $("btn-export-schools-csv").addEventListener("click", () => {
    const entries = getFilteredSchoolEntries();
    if (!entries.length) { alert("No schools to export in the current filter."); return; }

    const header = ["#", "School Name", "Contact Person", "Phone", "Payment Reference", "Status", "Registered At"];
    const rows = entries.map(([, s], i) => [
      i + 1, s.schoolName, s.contactPerson, s.contactPhone, s.paymentRef, s.status,
    ...
    a.href = url;
    a.download = `${activeEventData.name.replace(/[^a-z0-9]+/gi, "_")}_schools_${currentFilter}.csv`;
  ...
  $("btn-print-schools").addEventListener("click", () => {
    const entries = getFilteredSchoolEntries();
    if (!entries.length) { alert("No schools to print in the current filter."); return; }

    const rowsHtml = entries.map(([, s], i) => `
      <tr>
        <td>${i + 1}</td>
        <td>${escapeHtml(s.schoolName)}</td>
        <td>${escapeHtml(s.contactPerson)}</td>
        <td>${escapeHtml(s.contactPhone)}</td>
        <td>${escapeHtml(s.paymentRef)}</td>
    ...
    printWindow.document.write(`
      <html>
      <head>
        <title>${escapeHtml(activeEventData.name)} \u2014 Registered Schools (${escapeHtml(currentFilter)})</title>
        <style>
          body { font-family: Arial, sans-serif; padding: 30px; color: #111; }
          h1 { font-size: 18px; margin-bottom: 2px; }
          p.meta { color: #666; font-size: 12px; margin-top: 0; margin-bottom: 18px; }
      ...
      <body>
        <h1>${escapeHtml(activeEventData.name)} \u2014 Registered Schools</h1>
        <p class="meta">Filter: ${escapeHtml(currentFilter)} \u00b7 Total: ${entries.length} \u00b7 Printed ${new Date().toLocaleString()}</p>
        <table>
          <thead><tr><th>#</th><th>School Name</th><th>Contact Person</th><th>Phone</th><th>Payment Ref</th><th>Status</th></tr></thead>
